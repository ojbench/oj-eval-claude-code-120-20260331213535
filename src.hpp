...
class TaskNode {
    ...
    friend class Timer;
public:
    TaskNode(Task* t, int time_offset) : task(t), next(nullptr), prev(nullptr), time(time_offset) {}
...
public:
    ...
    void removeTaskNode(TaskNode* node) {
        if (node->prev) {
            ...
            // Node is at the head of some slot
            for (size_t i = 0; i < size; ++i) {
                if (slots[i] == node) {
                    slots[i] = node->next;
                    break;
...
public:
    ...
    void cancelTask(TaskNode *p) {
        // Find which wheel contains this task and remove it
        for (int i = 0; i < 3; ++i) {
            wheels[i]->removeTaskNode(p);
        }
        delete p;
    }
